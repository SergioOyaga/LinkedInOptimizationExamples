...
public class StatsPanel extends JPanel {
    ...
    /**
     * HashMap&lt;SquareButton, HashMap&lt;Boolean, Double&gt;&gt; with the current pheromone to print in the image.
     */
    private HashMap<SquareButton, HashMap<Boolean, Double>> pheromoneByButton;
    /**
    ...
    private void drawPheromones(Graphics2D g2) {
        //TODO: Check
        for(HashMap.Entry<SquareButton,HashMap<Boolean,Double>> buttonOrigEntry:this.pheromoneByButton.entrySet()){
            SquareButton origButton = buttonOrigEntry.getKey();
            for(HashMap.Entry<Boolean,Double> buttonDestEntry:buttonOrigEntry.getValue().entrySet()){
                double value = Math.tanh(buttonDestEntry.getValue())/2;

                g2.setColor( new Color(255,255,255,(int) (255*value)));
                if(buttonDestEntry.getKey()){
                    g2.setColor( new Color(255,0,0,(int) (255*value)));
                }
                int radious = (int)(Math.min(this.xLength, this.yLength)*value);
                g2.fillOval((origButton.getCol()+1) * this.xLength - radious, (origButton.getRow()+1) * this.yLength - radious,
                        radious*2, radious*2);
            }
        }
    }
